...
export function createVideo(
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            const imagePoster = document.createElement('picture');
            const Source = document.createElement("SOURCE");
            const image = document.createElement('img');
            image.src = `${DM_SERVER_URL}/eggsstage/${asset}?fit=constrain,&wid=1080&hei=720&qt=50`;
            Source.setAttribute("srcset", `${DM_SERVER_URL}/eggsstage/${asset}?fit=constrain,&wid=1080&hei=720&qt=50&fmt=webp`);
            Source.setAttribute("type", "image/webp");
            imagePoster.appendChild(Source);
