...
export default async function decorate(block: HTMLElement): Promise<void> {
  ...
  const logoImage: HTMLImageElement = fragmentHeaderData.querySelector('main>div>div>div>div:nth-of-type(1)>picture>img');
  const ctaText:HTMLParagraphElement = fragmentHeaderData.querySelector('main>div>div>div>div:nth-of-type(2)');
  ...
  <header class="header">
    ...
    <div class="right-container"> 
      <div class="find-dealer">
          ${ctaText?.outerHTML?  ctaText.outerHTML :  ' ' } 
       </div>  
       <div class="icon-find">
        ${getFindIcon()} 
      </div>
